...
def load_metadata(paths: list[Path]) -> dict[tuple[str, str], dict[str, str]]:
    metadata = {}
    for path in paths:
        ...
        if not all(isinstance(field, str) for field in (os_name, arch, sha256, url)):
            raise SystemExit(f"{path}: os, arch, sha256, and url must all be strings")
