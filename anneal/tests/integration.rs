...
fn with_toolchain_run_lock<T>(f: impl FnOnce() -> T) -> T {
    let target_dir = get_target_dir();
    fs::create_dir_all(&target_dir).expect("failed to create integration target directory");
    let lock_path = target_dir.join("anneal-toolchain-run.lock");
    let lock =
        fs::OpenOptions::new().create(true).write(true).open(&lock_path).unwrap_or_else(|e| {
            panic!("failed to open integration lock {}: {e}", lock_path.display())
        });

    lock.lock_exclusive().unwrap_or_else(|e| panic!("failed to lock {}: {e}", lock_path.display()));
    f()
}
...
fn run_single_phase(
    ...
    let assert = if config.mock.is_none() {
        with_toolchain_run_lock(|| ctx.run_anneal(&config))
    } else {
        ctx.run_anneal(&config)
    };
