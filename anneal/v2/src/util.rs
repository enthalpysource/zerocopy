...
impl DirLock {
    ...
    pub fn lock_exclusive(path: std::path::PathBuf) -> anyhow::Result<Self> {
    ...
    pub fn lock_shared(path: std::path::PathBuf) -> anyhow::Result<Self> {
        let file = Self::open_lock_file(&path)?;
        file.lock_shared()
...
pub fn patch_trace_files(dir: &std::path::Path, replacements: &[(&str, &str)]) -> anyhow::Result<()> {
    if dir.exists() {
...
where
    F: FnMut(&str, &indicatif::ProgressBar) -> anyhow::Result<()>,
{
