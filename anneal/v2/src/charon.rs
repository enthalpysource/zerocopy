...
mod tests {
    ...
    fn assert_fn_body(path: &std::path::Path, components: &[&str], expected_local: bool, expected_structured: bool) {
        let file = std::fs::File::open(path).unwrap();
        let crate_data: charon_lib::export::CrateData = serde_json::from_reader(file).unwrap();

        let fun = crate_data.functions.iter().find(|fun| {
            let name = &fun.item_meta.name;
            if name.0.len() != components.len() {
                return false;
            }
            for (i, elem) in name.0.iter().enumerate() {
                match elem {
                    charon_lib::types::PathElem::Ident(ident, _) => {
            ...
            true
        }).unwrap_or_else(|| panic!("Function with name path {:?} was not found/declared in LLBC!", components));

        assert_eq!(
            fun.item_meta.is_local, expected_local,
            "Local/External status mismatch for function {:?}", components
        );

        if expected_structured {
            assert!(
                matches!(fun.body, charon_lib::ullbc_ast::Body::Structured(_)),
                "Function {:?} was expected to contain a compiled structured implementation body!", components
            );
        } else {
            assert!(
                matches!(fun.body, charon_lib::ullbc_ast::Body::Opaque),
                "Function {:?} was expected to be Opaque (referred to but implementation body NOT included)!", components
            );
        }
    }
    ...
    fn test_charon_crates_io_dependency_not_chased_workspace_only() {
        ...
        // 3. Assert that the standard library function 'unwrap_or' is referred to but NOT translated (opaque).
        assert_fn_body(&llbc_path, &["core", "option", "Option", "unwrap_or"], false, false);
    }
    ...
    fn test_charon_crates_io_dependency_chasing() {
        ...
        let llbc_path_local = local_target.llbc_path(&locked_roots);
        // 5. Verify test_proj target translated (definition included).
        assert_fn_body(&llbc_path_local, &["test_proj", "call_dep"], true, true);

        // 6. Verify path dependency was CHASED and compiled as an independent root target with its own local definition!
        let llbc_path_dep = dep_target.llbc_path(&locked_roots);
        assert!(llbc_path_dep.exists());
        assert_fn_body(&llbc_path_dep, &["my_dep", "dep_fn"], true, true);
    }
    ...
    fn test_charon_crates_io_dependency_chasing() {
        ...
        assert_fn_body(&llbc_path_log, &["log", "log_enabled"], true, true);

        // 6. Deep Standard Library Check:
        // Investigating stdlib function implementation: Since stdlib functions (like 'unwrap_or')
        // are automatically lowered directly into the local MIR context of the calling crate,
        // we verify that we have its referred external declaration inside local LLBC (body opaque).
        let llbc_path_local = local_target.llbc_path(&locked_roots);
        assert_fn_body(&llbc_path_local, &["core", "option", "Option", "unwrap_or"], false, false);
    }
    ...
    fn test_charon_crates_io_dependency_chasing_offline_hermetic() {
        ...
        // 5. Verify that the chased crates.io dependency 'log' was compiled as an independent root target with local definition!
        let llbc_path_log = log_target.llbc_path(&locked_roots);
        assert!(llbc_path_log.exists());
        assert_fn_body(&llbc_path_log, &["log", "log_enabled"], true, true);
    }
    ...
    fn test_charon_stdlib_unwrap_or_not_chased_workspace_only() {
        ...
        // 1. The unwrap_or declaration matches and is referred to as external/opaque (definition body NOT included).
        assert_fn_body(&llbc_path, &["core", "option", "Option", "unwrap_or"], false, false);
    }
    ...
    fn test_charon_stdlib_unwrap_or_chased_dependency_following() {
        ...
        // Standard library functions are referred to as external, but generic items called directly are
        // lowered into the calling package's compiled body structure context (meaning local definition is Structured).
        assert_fn_body(&llbc_path, &["core", "option", "Option", "unwrap_or"], false, true);
    }
}
