...
mod tests {
    ...
    #[cfg(feature = "exocrate_tests")]
    fn copy_log_vendor_fixture(temp_dir: &tempfile::TempDir) {
        ...
        for entry in walkdir::WalkDir::new(&src_log) {
            let entry = entry.unwrap();
            let rel_path = entry.path().strip_prefix(&src_log).unwrap();
            let dest_path = dest_log.join(rel_path);
    ...
    fn test_charon_crates_io_dependency_chasing() {
        ...
        copy_log_vendor_fixture(&temp_dir);
