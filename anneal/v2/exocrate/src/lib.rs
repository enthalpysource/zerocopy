...
//!   - Fix up archive artifacts that depend on the current environment (e.g., rewriting absolute
...
//! to a hash of your `Cargo.toml`, `Cargo.lock`, `std::env::consts::OS`, and
//! `std::env::consts::ARCH`. This ensures that separate versions are designated for each:
//!
//! - Change to your crate/binary version number (in `Cargo.toml`);
//! - Change to your crate (internal) dependencies (in `Cargo.toml` and/or `Cargo.lock`);
//! - Different architectures that may require different precompiled binaries.
//!
//! To use this versioning strategy, write something like:
//!
//! ```rust
//! exocrate::config! {
...
//! other changes to `Cargo.toml` or `Cargo.lock`. This can result in proliferation of distinct
//! exocrate versions that developers need to clear out of your exocrate is particularly large.
//!
//! ## Custom versioning
//!
