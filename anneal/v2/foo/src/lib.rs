...
mod tests {
    ...
    fn test_install_overwrite_dir() {
        ...
        install(tar_v1.as_slice(), &dst, None).unwrap();
        ...
        install(tar_v2.as_slice(), &dst, Some(expected_hash)).unwrap();
    ...
    fn test_install_overwrite_dir_hash_mismatch() {
        ...
        let result = install(tar_v2.as_slice(), &dst, Some(bad_hash));
        assert!(result.is_err());
        assert_eq!(result.unwrap_err().kind(), std::io::ErrorKind::InvalidData);
